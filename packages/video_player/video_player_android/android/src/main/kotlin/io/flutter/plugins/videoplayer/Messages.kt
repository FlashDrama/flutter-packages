...
data class UrlLoadedEvent (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): UrlLoadedEvent {
      val duration = pigeonVar_list[0] as Long
      val width = pigeonVar_list[1] as Long
      val height = pigeonVar_list[2] as Long
      val rotationCorrection = pigeonVar_list[3] as Long
      return UrlLoadedEvent(duration, width, height, rotationCorrection)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
interface VideoPlayerInstanceApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.loadUrl$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val urlArg = args[0] as String
            val httpHeadersArg = args[1] as Map<String, String>
            val wrapped: List<Any?> = try {
